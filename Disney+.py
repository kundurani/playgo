...
BASE_URL = "https://dizifun6.com/disney"
PROXY_BASE_URL = "https://playproxy.roberparker19.workers.dev/"
TMDB_API_KEY = "d1feae88199a2a092c08827bc2f166a1"
...
# --- YENİ EKLENEN KISIM ---
# TMDb'de Türkçe ismiyle bulunamayan diziler için manuel eşleştirme haritası.
# Sol taraf dizifun'daki Türkçe isim, sağ taraf TMDb'de aranacak doğru (genellikle İngilizce) isim.
# Gelecekte sorun çıkaran bir dizi olursa, bu listeye eklemeniz yeterlidir.
MANUAL_TITLE_MAP = {
    "Anarşinin Evlatları": "Sons of Anarchy",
    "Cinayetten Paçayı Kurtarmak": "How to Get Away with Murder",
    "Katrina: Kıyamet Kasırgası": "Five Days at Memorial",
    "Dünyalar Savaşı": "War of the Worlds",
    # Örnek: "Türkçe Dizi Adı": "TMDb'deki Gerçek Adı"
}
# -----------------------------
...
def create_proxy_url(original_url):
    ...
    if not original_url:
        return None
    proxy_url = f"https://playproxy.roberparker19.workers.dev/?url={original_url}"
...
def sanitize_id(text):
    ...
    if not text:
        return "UNKNOWN"
    turkish_chars = { 'ç': 'c', 'Ç': 'C', 'ğ': 'g', 'Ğ': 'G', 'ı': 'i', 'I': 'I', 'İ': 'I', 'i': 'i', 'ö': 'o', 'Ö': 'O', 'ş': 's', 'Ş': 'S', 'ü': 'u', 'Ü': 'U' }
...
async def extract_gujan_m3u8(session, gujan_iframe_url):
    try:
        if gujan_iframe_url.startswith("//"): gujan_iframe_url = "https:" + gujan_iframe_url
        ...
        for script in scripts:
            ...
            m3u8_patterns = [ r'https?://[^"\s]+/hls/[^"/\s]+/playlist\.m3u8', r'https?://[^"\s]+\.m3u8', r'"(https?://gujan\.premiumvideo\.click/hls/[^"]+)"' ]
...
async def test_m3u8_url(session, url, timeout=15):
    try:
        async with session.get(url, timeout=aiohttp.ClientTimeout(total=timeout), allow_redirects=True) as response:
            ...
            if "premiumvideo.click" not in final_url: return False
            try:
                content = await response.content.read(4096)
                text = content.decode('utf-8', errors='ignore')
                logger.info(f"[DEBUG] İçerik ilk 100 karakter: {text[:100]}")
                if not text.strip().startswith("#EXTM3U"): return False
                suspicious_patterns = [ r"<html", r"<body", r"<title", r"error", r"not found", r"access denied", r"kerimkirac\.com", r"404", r"403", r"500" ]
                for pattern in suspicious_patterns:
                    if re.search(pattern, text, re.IGNORECASE): return False
                if content_length and int(content_length) < 50: return False
                if "master.m3u8" not in final_url and "playlist.m3u8" not in final_url: return False
                return True
...
async def get_series_from_page(session, page_num):
    ...
    has_next_page = False
    pagination_selectors = [ ".uk-pagination .uk-pagination-next", ".pagination .next", "a[href*='?p=']", ".uk-pagination a" ]
    ...
    if not has_next_page and series_links:
        ...
        if next_content:
            ...
            if next_links: has_next_page = True
...
async def get_series_metadata(session, series_url):
    """Dizi meta verilerini alır (YIL BİLGİSİ EKLENDİ)"""
    content = await fetch_page(session, series_url)
    if not content:
        return "Bilinmeyen Dizi", "", None
    soup = BeautifulSoup(content, 'html.parser')
    ...
    if season_buttons:
        ...
        for button in season_buttons:
            ...
            if not season_num:
                ...
                if button_id: season_num = str(button_id).split("-")[-1]
                else: continue
            ...
            if season_detail:
                ...
                for ep_element in episode_elements:
                    ...
                    if href:
                        if isinstance(href, str) and href.startswith("?"): full_url = f"{series_url}{href}"
                        else: full_url = fix_url(href)
        ...
        selectors = [ ".bolumler .bolumtitle a", ".episodes-list .episode a", ".episode-item a", "#season1 .uk-width-large-1-5 a" ]
...
async def extract_m3u8_from_episode(session, episode_url, season_num, episode_num):
    ...
    m3u8_url = None
    try:
        gujan_iframe_selectors = [ 'iframe[title="dizifunplay"]', 'iframe[id="altPlayerFrame"]', 'iframe[src*="gujan.premiumvideo.click"]' ]
        ...
        if not m3u8_url:
            ...
            iframe_selectors = [ 'iframe[title="playhouse"]', 'iframe[src*="playhouse.premiumvideo.click"]', 'iframe[src*="premiumvideo.click/player"]' ]
            playhouse_url = None
            file_id = None
            ...
            if not m3u8_url:
                ...
                iframe_selectors_fallback = [ "iframe#londonIframe", "iframe[src*=premiumvideo]", "iframe[data-src*=premiumvideo]", "iframe[src*=player]", "iframe" ]
                for selector in iframe_selectors_fallback:
                    iframe_element = soup.select_one(selector)
                    if iframe_element:
                        src = iframe_element.get("src")
                        if not src or src == "about:blank": src = iframe_element.get("data-src")
...
def search_tmdb_series(series_name, year=None):
    try:
        clean_name = series_name.replace('ı', 'i').replace('İ', 'I').replace('ğ', 'g').replace('Ğ', 'G').replace('ü', 'u').replace('Ü', 'U').replace('ş', 's').replace('Ş', 'S').replace('ö', 'o').replace('Ö', 'O').replace('ç', 'c').replace('Ç', 'C')
        url = f"{TMDB_BASE_URL}/search/tv"
        params = { 'api_key': TMDB_API_KEY, 'query': clean_name, 'language': 'tr-TR', 'page': 1 }
        if year:
        ...
        if response.status_code == 200:
            ...
            if results:
                ...
                original_name = best_match.get('name', series_name)
                first_air_date = best_match.get('first_air_date', '')
                logger.info(f"[TMDb] '{series_name}' -> ID: {tmdb_id}, Adı: {original_name}")
                return tmdb_id, original_name, first_air_date
            else:
                logger.warning(f"[TMDb] '{series_name}' için sonuç bulunamadı")
                return None, series_name, None
        else:
            logger.error(f"[TMDb] API hatası: {response.status_code}")
            return None, series_name, None
    except Exception as e:
        logger.error(f"[TMDb] Arama hatası: {e}")
        return None, series_name, None
    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(limit=10)) as session:
        with open(output_filename, "w", encoding="utf-8") as f:
            ...
            for series_url in all_series_links:
                try:
                    title, logo_url, year = await get_series_metadata(session, series_url)
                    logger.info(f"\n[+] İşleniyor: {title} ({year or 'Yıl Yok'})")

                    # --- DEĞİŞİKLİK BURADA BAŞLIYOR ---
                    # Önce manuel haritayı kontrol et. Orada varsa onu kullan, yoksa orijinal başlığı kullan.
                    search_query = MANUAL_TITLE_MAP.get(title, title)
                    if search_query != title:
                        logger.info(f"[*] Manuel eşleştirme kullanılıyor: '{title}' -> '{search_query}'")
                    
                    # TMDb aramasını dizi başına SADECE BİR KERE, doğru sorguyla yap
                    tmdb_id, tmdb_name, _ = search_tmdb_series(search_query, year)
                    ...
                    for i, result in enumerate(results):
                        ...
                        episode_title = f"S{season_num:02d}E{normalized_episode_num:02d}"

                        # --- DEĞİŞİKLİK BURADA ---
                        # Başta bulunan TMDb ID ve ismini tüm bölümler için kullan.
                        final_id = str(tmdb_id) if tmdb_id else sanitize_id(title)
                        final_name = tmdb_name if tmdb_id else title
                        
                        f.write(
                            f'#EXTINF:-1 tvg-id="{final_id}" '
                            f'tvg-name="{final_name}" '
                            f'group-title="Disney+" '
                            f'tvg-logo="{logo_url}" '
                            f'episode-title="{episode_title}" '
                            f'season-number="{season_num}" '
                            f'episode-number="{normalized_episode_num}",'
                            f'{final_name} {episode_title}\n'
                        )
                        f.write(m3u8_url.strip() + "\n")
                        logger.info(f"[✓] {final_name} {episode_title} eklendi.")
