...
BASE_URL = "https://dizifun6.com/tabii-dizileri"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if results:
                ...
                original_name = best_match.get('original_name', series_name)
                first_air_date = best_match.get('first_air_date', '')
                
                logger.info(f"[TMDb] '{series_name}' -> ID: {tmdb_id}, Original: {original_name}")
...
def get_tmdb_series_details(tmdb_id):
    """TMDb'den dizi detaylarını alır"""
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(f"[TMDb] Detay hatası: {response.status_code}")
            return None
            
    except Exception as e:
        logger.error(f"[TMDb] Detay alma hatası: {e}")
        return None
...
async def get_series_from_page(session, page_num):
    ...
    # Ana sayfadan tüm dizileri çek (sayfalama yok)
    diziler_url = BASE_URL
    logger.info(f"Ana sayfa alınıyor: {diziler_url}")

    content = await fetch_page(session, diziler_url)
    if not content:
        logger.warning(f"[!] Ana sayfa alınamadı.")
        return [], False
    ...
    for selector in selectors:
        ...
        logger.info(f"Selector '{selector}' ile {len(link_elements)} link bulundu")
        ...
        if series_links:  # Eğer link bulunduysa diğer selector'ları deneme
            break
...
async def get_series_from_homepage():
    """Ana sayfadan tüm dizi listesini alır"""
    async with aiohttp.ClientSession() as session:
        all_series_links = []
        
        # Sadece ana sayfayı tara (sayfalama yok)
        series_links, has_next_page = await get_series_from_page(session, 1)

        if not series_links:
            logger.error("[!] Ana sayfadan dizi bulunamadı!")
            return []
        ...
        logger.info(f"[✓] Toplam {len(all_series_links)} benzersiz dizi linki toplandı.")
...
async def process_series(all_series_links, output_filename="Tabii.m3u"):
    """Tüm dizileri tek bir dosyaya yazar - TMDb entegrasyonu ile"""
        ...
        with open(output_filename, "w", encoding="utf-8") as f:
            ...
            for series_url in all_series_links:
                try:
                    ...
                    # TMDb'den dizi bilgilerini al
                    tmdb_id, tmdb_name, first_air_date = search_tmdb_series(title)
                    
                    if tmdb_id:
                        logger.info(f"[TMDb] Dizi bulundu: {tmdb_name} (ID: {tmdb_id})")
                    else:
                        logger.warning(f"[TMDb] Dizi bulunamadı: {title}")
                    ...
                    for i, result in enumerate(results):
                        ...
                        # Yeni M3U formatı - sadece dizi adı ve sezon bilgileri
                        series_name = tmdb_name if tmdb_name else title
                        episode_title = f"S{season_num:02d}E{normalized_episode_num:02d}"
                        
                        # TMDb ID ile birlikte M3U formatı
                        if tmdb_id:
                            f.write(
                                f'#EXTINF:-1 tvg-id="{tmdb_id}" '
                                f'tvg-name="{series_name}" '
                                f'group-title="Tabii Diziler" '
                                f'tvg-logo="{logo_url}" '
                                f'episode-title="{episode_title}" '
                                f'season-number="{season_num}" '
                                f'episode-number="{normalized_episode_num}",{series_name} {episode_title}\n'
                            )
                        else:
                            # TMDb ID yoksa sadece dizi adı
                            f.write(
                                f'#EXTINF:-1 tvg-name="{series_name}" '
                                f'group-title="Tabii Diziler" '
                                f'tvg-logo="{logo_url}" '
                                f'episode-title="{episode_title}" '
                                f'season-number="{season_num}" '
                        ...
                        logger.info(f"[✓] {series_name} {episode_title} eklendi.")
